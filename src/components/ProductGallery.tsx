      ...
      <div className="flex md:flex-col gap-4 overflow-x-auto md:w-24 shrink-0 pb-4 md:pb-0">
        {images.map((img, i) => (
          <div 
            ...
            className={`w-20 h-24 md:w-24 md:h-32 bg-gray-100 relative cursor-pointer border-2 transition shrink-0 ${
              activeIdx === i ? 'border-gold shadow-md' : 'border-transparent hover:border-gray-300'
            }`}
          >
            {img.startsWith('data:') || img.startsWith('/') ? (
              <img src={img} alt={`${name} thumbnail ${i}`} className="w-full h-full object-cover" />
            ) : (
